...
class AddMedecineDoctorCommand(BaseCommand, MedecineDoctorParser):
    ...
    def run(self, args):
        ...
        new_generalist = md.GeneralistDoctor(**self.values)
...
class ListMedecineDoctorCommand(BaseCommand):
    ...
    def __init__(self):
        self.query = meta.session.query(md.GeneralistDoctor)
        ...
        if args:
            for keyword in args:
                ...
                query = query.filter(or_(\
                                md.GeneralistDoctor.lastname.ilike(keyword),
                                md.GeneralistDoctor.firstname.ilike(keyword),
                                md.GeneralistDoctor.city.ilike(keyword)))
...
class UpdateMedecineDoctorCommand(BaseCommand, MedecineDoctorParser):
    ...
    def run(self, args):
        ...
        doc = self.query.filter(md.GeneralistDoctor.id ==
                                options.md_id).one()
